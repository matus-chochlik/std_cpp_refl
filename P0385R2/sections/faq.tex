...
\begin{minted}{cpp}
std::string(std::meta::get_base_name_v<$reflect(std::pair)>);
\end{minted}
...
\verb@meta::get_pointer_v<$reflect(variable)>@ or\\\verb@decltype(var)@ vs.
\verb@meta::get_reflected_type_t<meta::get_type_m<$reflect(var)>>@, etc.}
...
\begin{minted}{cpp}
get_element_m<get_data_members_m<$reflect(my_class)>, 0>;
\end{minted}
...
\item
\begin{minted}{cpp}
meta::object mo = $reflect(my_ns::my_class);
