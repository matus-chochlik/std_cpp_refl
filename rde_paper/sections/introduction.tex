...
// the are distinguishable
assert(!std::is_same_v<type_a, type_b>, "");
assert(!std::is_same_v<identity<type_a>, identity<type_b>>, "");

// they can be stored in named "variables" ...
using x = type_a;

// ... while maintaining their identity
assert(std::is_same_v<type_a, x>, "");
...
using y = identity<type_a>::result;
\end{minted}


On the other hand a namespace, a typedef or a template parameter is not.
...
assert(std::is_same_v<bar, baz>, "");
assert(!std::is_same_v<identity<bar>, identity<baz>>, "");
