...
metaprograms even if it were not possible with the base-level declarations.
...
using x = my_meta_algorithm<std>::result; // Error

using y = my_meta_algorithm<reflexpr(std)>::result; // OK
\end{minted}
...
can be implemented on top of reflection instead of each of them using their own
